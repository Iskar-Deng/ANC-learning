...
PHENOMENON_ID = "5.4"
PHENOMENON_NAME = "anc_ext_gen_marker"
...
def foil_marker_for_gen(row: Dict[str, Any] | None, source_index: int) -> tuple[str, str]:
    ...
    if index % 2:
        return "", "delete_anc_external_genitive_ge"
    return "ca", "replace_anc_external_genitive_ge_with_ca"
...
def perturb(
    ...
    if template is None:
        return {
            "skip": True,
            "skip_reason": "source_index_outside_5_4_profile_ranges",
            "good": good_sentence,
    ...
    if anc_verb.marker != good_marker:
        return {
            "skip": True,
            "skip_reason": "anc_external_gen_marker_does_not_match_expected_ge",
            "good": good_sentence,
            "tokens": tokens,
    ...
    bad_marker, perturbation = foil_marker_for_gen(row, source_index)
    bad_tokens = tokens[:]
    ...
    return {
        "bad": " ".join(bad_tokens),
        "target_role": "ANC_EXT_GEN",
        "target_index": anc_verb.index,
        "target_token": anc_verb.token,
        "target_stem": anc_verb.stem.lower(),
        "good_value": good_marker,
