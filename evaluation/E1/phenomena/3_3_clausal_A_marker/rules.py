...
PHENOMENON_ID = "3.3"
PHENOMENON_NAME = "clausal_A_marker"
...
def anc_a_foil_marker(language_config: Dict[str, Any]) -> str:
    strategy = language_config["strategy"]
    if strategy == "erg-poss":
        return "ob"
    if strategy in {"sent", "poss-acc", "nomn"}:
        return "ge"
...
def perturb(
    ...
    if parsed is None or parsed.embedded_construction != "tv":
        return {
            ...
            "good_value": marker_value(language_config["FIN_A_MARK"]),
        }

    assert parsed.embedded_a is not None
    assert parsed.embedded_p is not None

    good_mark = language_config["FIN_A_MARK"]
    bad_mark = anc_a_foil_marker(language_config)
    target_index = parsed.embedded_a.head_index
    ...
    return {
        ...
        "embedded_a_head_index": target_index,
        "embedded_p_span": parsed.embedded_p.text,
        "good_value": marker_value(good_mark),
        "bad_value": bad_mark,
        "strategy": language_config["strategy"],
        "template": parsed.template_name,
        "perturbation": "replace_clausal_complement_a_marker_with_anc_a_marker",
    }
