...
def perturb(
    ...
    if template is None:
        return {
            "skip": True,
            "skip_reason": "source_index_outside_4_5_profile_ranges",
            "good": good_sentence,
    ...
    bad_tokens = tokens[:]
    bad_tokens[target_index], bad_tokens[anc_verb.index] = (
        bad_tokens[anc_verb.index],
        bad_tokens[target_index],
    )

    good_value = "SV" if target_index < anc_verb.index else "VS"
