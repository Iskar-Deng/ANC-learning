...
def bad_anc_p_marker(language_config: Dict[str, Any], good_marker: str) -> str:
    strategy = language_config["strategy"]
    ...
    raise ValueError(
        f"Unsupported ANC P marker perturbation: {strategy=} {good_marker=}"
    )
...
def perturb(
    ...
    if template is None:
        return {
            "skip": True,
            "skip_reason": "source_index_outside_4_4_profile_ranges",
            "good": good_sentence,
    ...
    target_index = find_unique_expected_head_index(tokens, pseudo_args.p_head)
    ...
    actual_marker = marker_for_expected_head(target_token, pseudo_args.p_head)
    if actual_marker is None:
        return {
            "skip": True,
            "skip_reason": "anc_p_target_does_not_match_pseudo_head",
            "good": good_sentence,
            "tokens": tokens,
    ...
    good_marker = expected_anc_p_marker(language_config)
    ...
    bad_marker = bad_anc_p_marker(language_config, good_marker)
    bad_tokens = tokens[:]
    ...
    return {
        ...
        "a_validation_token": tokens[a_index],
