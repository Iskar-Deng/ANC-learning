...
def foil_marker_for_p(good_marker: str, row: Dict[str, Any] | None, source_index: int) -> tuple[str, str]:
    ...
    if good_marker == "":
        ...
        return bad_marker, f"add_{bad_marker}_to_anc_external_p_head"

    if good_marker == "ca":
        if index % 2:
            return "", "delete_anc_external_p_ca"
        return "ge", "replace_anc_external_p_ca_with_ge"

    bad_marker = "ca" if good_marker != "ca" else "ge"
    return bad_marker, f"replace_anc_external_p_{good_marker}_with_{bad_marker}"
...
def perturb(
    ...
    if template is None:
        return {
            "skip": True,
            "skip_reason": "source_index_outside_5_3_profile_ranges",
            "good": good_sentence,
    ...
    if anc_verb.marker != good_marker:
        return {
            "skip": True,
            "skip_reason": "anc_external_p_marker_does_not_match_expected_value",
            "good": good_sentence,
            "tokens": tokens,
    ...
    bad_marker, perturbation = foil_marker_for_p(good_marker, row, source_index)
    bad_tokens = tokens[:]
